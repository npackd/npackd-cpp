...
CHECK_TYPE_SIZE("void*" NPACKED_PTR_SIZE BUILTIN_TYPES_ONLY)
if(NPACKED_PTR_SIZE EQUAL 8)
...
file(STRINGS "appveyor.yml" READ_BUILD_NUMBER REGEX "version:.+" LIMIT_COUNT 1)
...
macro(generateRC tgt adm)
	set(PROGRAM_NAME "${tgt}")
	set(ADMIN_LEVEL "asInvoker")
	if(${adm})
		set(ADMIN_LEVEL "requireAdministrator")
	endif()
	file(COPY ${CMAKE_SOURCE_DIR}/cmake/app.ico DESTINATION ${CMAKE_CURRENT_BINARY_DIR})
	set(OUTPUT_FILE_NAME "${tgt}.exe")
    configure_file(${CMAKE_SOURCE_DIR}/cmake/app.rc.in ${CMAKE_CURRENT_BINARY_DIR}/app.rc @ONLY)
	configure_file(${CMAKE_SOURCE_DIR}/cmake/app.manifest.in ${CMAKE_CURRENT_BINARY_DIR}/app.manifest @ONLY)
	if(MSVC)
		add_custom_command(TARGET ${tgt} PRE_BUILD COMMAND powershell -Command "(Get-Content '${CMAKE_CURRENT_BINARY_DIR}/app.rc') -replace '[a-zA-Z0-9 _\\-.]+.exe','$(TargetFileName)' | Set-Content '${CMAKE_CURRENT_BINARY_DIR}/app.rc'")
		target_link_options(${tgt} PUBLIC "/level='${ADMIN_LEVEL}'")
		target_link_options(${tgt} PUBLIC "/MANIFEST:NO")
	endif()
endmacro(generateRC)
