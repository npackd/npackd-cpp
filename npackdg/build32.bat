...
copy src\CrystalIcons_LICENSE.txt dist
if %errorlevel% neq 0 exit /b %errorlevel%

copy src\LICENSE.txt dist
if %errorlevel% neq 0 exit /b %errorlevel%

rem TODO exeproxy

C:\Windows\System32\xcopy.exe dist dist-debug /E /I /H /Y
if %errorlevel% neq 0 exit /b %errorlevel%
...
copy "%DRMINGW%\bin\exchndl.dll" dist
if %errorlevel% neq 0 exit /b %errorlevel%

copy "%DRMINGW%\bin\mgwhelp.dll" dist
if %errorlevel% neq 0 exit /b %errorlevel%

copy "%DRMINGW%\bin\dbghelp.dll" dist
if %errorlevel% neq 0 exit /b %errorlevel%

copy "%DRMINGW%\bin\symsrv.dll" dist
if %errorlevel% neq 0 exit /b %errorlevel%

copy "%DRMINGW%\bin\symsrv.yes" dist
if %errorlevel% neq 0 exit /b %errorlevel%

7z a build\Npackd%bits%-debug-%version%.zip dist-debug\* -mx9	
if %errorlevel% neq 0 exit /b %errorlevel%

7z a build\Npackd%bits%-%version%.zip dist\* -mx9	
if %errorlevel% neq 0 exit /b %errorlevel%

copy src\npackdg%bits%.aip build
if %errorlevel% neq 0 exit /b %errorlevel%

copy src\app.ico build
if %errorlevel% neq 0 exit /b %errorlevel%
